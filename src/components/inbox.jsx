...
class Inbox extends Component{
    ...
    componentDidUpdate(prevProps){
        if(prevProps.inboxMessages.length !== this.props.inboxMessages.length){
    ...
    render () {
        return(
            <div>
                {this.props.inboxMessages.map((messages,index) => {
                    return(
                        <div key={index}>
                                ...
                                <div className='m-l-5 message-card-items h-60' onClick={() => this.updateOnClick(messages)}>
                                    ...
                                    <div className={`w-95 m-l-5 fs-12 ${messages.unread ? 'colorBlue' : ''}`}>
                                        <div className='row m-l-0'>
                                            ...
                                            <div className='row col-md-1 displayNone'><span onClick={(e)=>{e.stopPropagation(); this.props.deleted(messages,'inbox');}}><i className="fa fa-trash trashIcon"></i></span></div>
                                        </div>
                                    </div>
